 ...
 */
NTSTATUS
CCaptureSinkMXF::SetState(__in KSSTATE State)
{
    PAGED_CODE();
 ...
 */
NTSTATUS CCaptureSinkMXF::ConnectOutput(__in PMXF sinkMXF)
{
    PAGED_CODE();
 ...
 */
NTSTATUS CCaptureSinkMXF::DisconnectOutput(__in PMXF sinkMXF)
{
    PAGED_CODE();
 ...
 */
NTSTATUS CCaptureSinkMXF::PutMessage(__in PDMUS_KERNEL_EVENT pDMKEvt)
{
...
NTSTATUS CCaptureSinkMXF::ParseFragment(PDMUS_KERNEL_EVENT pDMKEvt)
{
    ...
    if (pPrevData)
    {
        ...
        {
            // In ParseOneByte() above, if we get the IS_SYSEX_END element we will
            // Process the queue before returning. When we do, we also change pPrefData
            // to the last midi event. It needs to be freed because it's pNextEvt
            // is NULL (so it didn't make it into the if above). 
            if (pPrevData == pDMKEvt)
            {
                (void) m_AllocatorMXF->PutMessage(pPrevData);
            }
        }
    }
...
NTSTATUS CCaptureSinkMXF::ParseSysExByte(BYTE aByte,PDMUS_KERNEL_EVENT *ppDMKEvt,REFERENCE_TIME refTime)
{
    ...
    if (STATUS_STATE(pDMKEvt))  //  delete running status
    {
...
NTSTATUS CCaptureSinkMXF::ParseEOXByte(BYTE aByte,PDMUS_KERNEL_EVENT *ppDMKEvt,REFERENCE_TIME refTime)
{
    ...
    if (!pDMKEvt)
    {
        _DbgPrintF(DEBUGLVL_BLAB, ("ParseEOXByte: invalid ppDMKEvt"));
        return STATUS_INVALID_PARAMETER_2;
    }
...
NTSTATUS CCaptureSinkMXF::ParseRTByte(BYTE aByte,PDMUS_KERNEL_EVENT *ppDMKEvt,REFERENCE_TIME refTime)
{
    ...
    if (!pDMKEvt)
    {
        _DbgPrintF(DEBUGLVL_BLAB, ("ParseRTByte: invalid ppDMKEvt"));
        return STATUS_INVALID_PARAMETER_2;
    }
...
VOID CCaptureSinkMXF::InsertListEvent(PDMUS_KERNEL_EVENT pDMKEvt)
{
    ...
    if (!pDMKEvt)
    {
        _DbgPrintF(DEBUGLVL_BLAB, ("InsertListEvent: invalid pDMKEvt"));
        return;
    }
