	...
	unsigned limit)
{
	...
	*used = t;
	...
	if ( t > limit)
	{
		...
		USB2LIB_DbgPrintUlong("Allocation of (*used-newval) ", (*used-newval));
        USB2LIB_DbgPrintUlong("Allocation of newval ", newval);
        USB2LIB_DbgPrintUlong("Allocation of limit ", limit);
        USB2LIB_DbgPrintMsg("Allocation check failed");
		ok = 0;
	}
	return ok;
}
	...
	int			frame_bias)
{
	...
	for (i=0;i < ep->num_starts; i++)
	{
		...
		if (ep->mytt->num_starts[f][m] + 1 > 16) {
			sched_error("too many SSs in microframe");
			retv = 0;
		}
...
//		((i < *changed_eps) && !curr_ep->moved_this_req) || // have seen this endpoint this pass
	...
	{	// Update NEW changed endpoint
		...
		{
			sched_error("end of new xact too late");
			*err = 0;
		}
...
********/
int Common_frames(PUSB2_EP a, PUSB2_EP b)
{
	PUSB2_EP maxep, minep;
...
int Deallocate_endpoint_budget(
	PUSB2_EP ep,					// endpoint that needs to be removed (bus time deallocated)
	PUSB2_EP changed_ep_list[],	// pointer to array to set (on return) with list of
									// changed endpoints
									...
									// allocations.
{
	...
	*/
	...
	PUSB2_EP curr_ep, last_ep, p, head, gap_ep;

	// check that this endpoint is already allocated
	if (! ep->calc_bus_time)
	{
		sched_error("endpoint not allocated");
		return 0;
	}
					...
					if (shift_time > 0)
					{
						moved = Move_ep(
							...
							*max_changed_eps,
							&move_status);
						if (! move_status)
							sched_error("deallocation move failure!!");  //<<few things can go wrong here, but num eps could >>
				...
				if (shift_time > 0)
				{
					moved = Move_ep(
						...
						*max_changed_eps,
						&move_status);
					if (! move_status)
						sched_error("deallocate move failure 2");  // <<few things, but num eps could fail here>>
				}
				...
				if (shift_time > 0)
				{
					moved = Move_ep(
						...
						*max_changed_eps,
						&move_status);
					if (! move_status)
						sched_error("deallocate move failure 3"); //  << few things, but num eps could fail here>>
				}
...
int Allocate_time_for_endpoint(
	PUSB2_EP ep,					// endpoint that needs to be configured (bus time allocated)
	PUSB2_EP changed_ep_list[],	// pointer to array to set (on return) with list of
									// changed endpoints
	int	*max_changed_eps			// input: maximum size of (returned) list
									// on return: number of changed endpoints
	)
{
	...
	if (ep->calc_bus_time)
	{
		sched_error("endpoint already allocated");
		return 0;
	}
...
//	if ((ep->speed == HSSPEED) && (ep->mytt->myHC->speed == HSSPEED)) {
	...
	if (ep->ep_type == isoch)
	{
		...
		{
			sched_error("low speed isoch illegal"); // illegal, LS isoch
			return 0;
		}
	} else
	{ // interrupt
	...
	for (i=0; ep->start_frame + i < MAXFRAMES; i += ep->actual_period)
	{
		...
		if (ep->mytt->frame_budget[ep->start_frame + i].allocated_large)
		{
			if (ep->calc_bus_time >= LARGEXACT)
			{
				sched_error("too many large xacts");  // only one large transaction allowed in a frame.
				return 0;
			}
		}
	...
	{
//		sched_error("start time %d past end of frame", ep->start_time + ep->calc_bus_time);
		ep->calc_bus_time = 0;
		return 0;
	}
	...
	//***
		...
		if (ep->ep_type == isoch)
		{
			...
			//if ((ep->start_time + ep->calc_bus_time) > FS_MAX_PERIODIC_ALLOCATION)
			//{
			//	sched_error("new xact too late in frame");
