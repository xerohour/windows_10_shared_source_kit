 ...
 *
 */
NTSTATUS CPackerMXF::SetState(__in KSSTATE State)    
{   
    PAGED_CODE();
 ...
 */
NTSTATUS CPackerMXF::ConnectOutput(__in PMXF sinkMXF)
{
    PAGED_CODE();
 ...
 * Fail; this filter is an MXF sink only
 */
NTSTATUS CPackerMXF::DisconnectOutput(__in PMXF sinkMXF)
{
    PAGED_CODE();
 ...
 */
NTSTATUS CPackerMXF::PutMessage(__in PDMUS_KERNEL_EVENT pDMKEvtHead)
{
...
NTSTATUS CPackerMXF::ProcessQueues()
{
    ...
    while (m_DMKEvtHead)
    {
        ...
        while (cbDest < m_MinEventSize)
        {
            if (!cbDest || !pbDest)
            {
                _DbgPrintF(DEBUGLVL_VERBOSE,("ProcessQ: returning, no dest buffer"));
                return ntStatus;
            }
...
PBYTE CPackerMXF::GetDestBuffer(PULONG pcbDest)
{
    ...
    //
    // If there is no buffer address or no bytes to process then forget
...
{
    return m_IrpStream->
        ChangeOptionsFlags(0, (ULONG)(~KSSTREAM_HEADER_OPTIONSF_DATADISCONTINUITY),   
                           0, 0xFFFFFFFF);
}
