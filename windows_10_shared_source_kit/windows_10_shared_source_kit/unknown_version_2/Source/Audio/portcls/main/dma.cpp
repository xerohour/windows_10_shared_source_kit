...
#ifdef DEFINE_DMA_CHANNEL_SLAVE
/*****************************************************************************
 * IDmaChannelInit
 *****************************************************************************
 * Interface for dma channel with Init. Derived from IDmaChannelSlave
 */
 ...
 * IDmaChannelInit
 *****************************************************************************
 * Interface for dma channel with Init. Derived from IDmaChannel
 */
DECLARE_INTERFACE_(IDmaChannelInit,IDmaChannel)
{
    DEFINE_ABSTRACT_UNKNOWN()           // For IUnknown
...
)
{
    ...
    m_MaxMapRegisters = DeviceDescription->MaximumLength / PAGE_SIZE + 1;
    ...
    {
        if (m_MaxMapRegisters * PAGE_SIZE < DeviceDescription->MaximumLength)
        {
            m_MaxBufferSize = m_MaxMapRegisters * PAGE_SIZE;
        }
        else
        {
...
)
{
    ...
    CDmaChannel *that = (CDmaChannel *)pvDmaChannel;

    if( that )
    {
        return that->m_MaxMapRegisters;
    }
 ...
 * CDmaChannel::Start()
 *****************************************************************************
 * Removed spinlock and event, replaced by single mutex.  Must be in non-pageable code, 
 * since IRQL is raised to DISPATCH_LEVEL.
 *
 */
STDMETHODIMP_(NTSTATUS)
CDmaChannel::
...
)
{
    if (BufferSize > m_AllocatedBufferSize)
        BufferSize =  m_AllocatedBufferSize;
